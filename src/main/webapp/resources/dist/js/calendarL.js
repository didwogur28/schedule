...
function calendarInitL() {
  ...
  function renderCalender(thisMonth) {
    ...
    for (var i = prevDate - prevDay + 1; i <= prevDate; i++) {
      calendar.innerHTML = calendar.innerHTML + '<div id="prev'+i+'" class="day prev disable">' + i + '</div>'
    }
    // 이번달
    for (var i = 1; i <= nextDate; i++) {
      calendar.innerHTML = calendar.innerHTML + '<div id="cur'+i+'" class="day current">' + i + '</div>'
    }
    // 다음달
    for (var i = 1; i <= (7 - nextDay == 7 ? 0 : 7 - nextDay); i++) {
      calendar.innerHTML = calendar.innerHTML + '<div id="next'+i+'" class="day next disable">' + i + '</div>'
    }
